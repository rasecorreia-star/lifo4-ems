...
export default function DeviceDiscovery({ onDeviceAdded }: DeviceDiscoveryProps) {
  ...
  useEffect(() => {
    const checkNewDevices = async () => {
      try {
        const response = await fetch(buildApiUrl('/discovery/new'));
        if (response.ok) {
  ...
  const scanForDevices = async () => {
    setIsScanning(true);
    try {
      const response = await fetch(buildApiUrl('/discovery/scan'), {
        method: 'POST',
      });
  ...
  const addDevice = async (device: DiscoveredDevice) => {
    ...
    try {
      const response = await fetch(buildApiUrl('/discovery/add'), {
        method: 'POST',
