...
/**
 * @deprecated Use direct api.post/api.get calls with /api/v1/optimization/* endpoints instead
 * These wrappers use incorrect paths that don't match backend routes
 */
export const optimizationApi = {
  start: (systemId: string, config: OptimizationConfig) =>
    api.post(`/api/v1/optimization/${systemId}/decision`, config),

  stop: (systemId: string) =>
    api.post(`/api/v1/optimization/${systemId}/stop`),

  getSchedule: (systemId: string) =>
    api.get(`/api/v1/optimization/${systemId}/schedule`),

  getRecommendation: (systemId: string) =>
    api.get(`/api/v1/optimization/${systemId}/decision/current`),

  getStrategies: () =>
    api.get('/api/v1/optimization/strategies'),

  getConfig: (systemId: string) =>
    api.get(`/api/v1/optimization/config/default`),

  updateConfig: (systemId: string, config: Record<string, unknown>) =>
    api.put(`/api/v1/optimization/${systemId}/config`, config),

  getResults: (systemId: string) =>
    api.get(`/api/v1/optimization/${systemId}/results`),

  runOptimization: (systemId: string) =>
    api.post(`/api/v1/optimization/${systemId}/run`),
};

// ============================================
// BLACK START API — DEPRECATED
// ============================================
...
/**
 * @deprecated Use direct api calls with /api/v1/optimization/grid-services/black-start/* endpoints instead
 */
export const blackStartApi = {
  initialize: (systemId: string, config: BlackStartConfig) =>
    api.post(`/api/v1/optimization/grid-services/black-start/process`, config),

  trigger: (systemId: string) =>
    api.post(`/api/v1/optimization/${systemId}/grid-services/black-start/engage`),

  reconnect: (systemId: string) =>
    api.post(`/api/v1/optimization/grid-services/black-start/restoration-time`),

  getStatus: (systemId: string) =>
    api.get(`/api/v1/optimization/${systemId}/grid-services/black-start/status`),

  getHistory: (systemId: string, limit?: number) =>
    api.get(`/api/v1/optimization/grid-services/black-start/state-history`, { params: { limit } }),
};

// ============================================
// GRID INTEGRATION API — DEPRECATED
// ============================================
...
/**
 * @deprecated Use direct api calls with /api/v1/optimization/grid-services/* endpoints instead
 */
export const gridApi = {
  initialize: (systemId: string, config: HybridSystemConfig) =>
    api.post(`/api/v1/optimization/grid-services/select-mode`, config),

  getPowerFlow: (systemId: string) =>
    api.get(`/api/v1/optimization/${systemId}/grid-services/status`),

  setControlMode: (systemId: string, mode: string) =>
    api.post(`/api/v1/optimization/grid-services/select-mode`, { gridState: { gridConnected: true, frequency: 60, voltage: 380 } }),

  getControlModes: () =>
    api.get('/api/v1/optimization/grid-services/current-mode'),

  getEnergyHistory: (systemId: string, start: string, end: string) =>
    api.get(`/api/v1/optimization/${systemId}/grid-services/status`, { params: { start, end } }),
};
