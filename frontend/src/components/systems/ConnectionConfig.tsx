...
export default function ConnectionConfig({ systemId, systemName, onConnectionChange }: ConnectionConfigProps) {
  ...
  const loadConfig = async () => {
    try {
      const response = await fetch(`http://localhost:3001/api/v1/systems/${systemId}/connection`);
      if (response.ok) {
  ...
  const testConnection = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/api/v1/systems/${systemId}/connection/test`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(config),
      });
  ...
  const toggleConnection = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/api/v1/systems/${systemId}/connection/${newEnabled ? 'connect' : 'disconnect'}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(config),
      });
