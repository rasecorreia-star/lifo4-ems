...
const generateLoadProfile = (): LoadData[] => {
  const data: LoadData[] = [];
  for (let i = 0; i < 24; i++) {
    const hour = `${i.toString().padStart(2, '0')}:00`;
    const baseline = 80 + Math.sin((i - 6) * Math.PI / 12) * 40;
    const drActive = i >= 17 && i <= 20;
    const target = drActive ? baseline * 0.7 : baseline;
    const actual = drActive
      ? baseline * (0.65 + Math.random() * 0.1)
...
const generatePerformanceHistory = (): PerformanceData[] => {
  ...
  return months.map((month) => ({
    month,
    eventsParticipated: 8 + Math.floor(Math.random() * 5),
    totalReduction: 150 + Math.floor(Math.random() * 100),
    earnings: 2000 + Math.floor(Math.random() * 1500),
...
export default function DemandResponse() {
  ...
  const loadProfile = useMemo(() => generateLoadProfile(), []);
