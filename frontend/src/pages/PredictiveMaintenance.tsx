...
const generateMaintenanceHistory = (): MaintenanceHistory[] => {
  const months = ['Jul', 'Ago', 'Set', 'Out', 'Nov', 'Dez', 'Jan'];
  return months.map((date) => ({
    date,
    predicted: 2 + Math.floor(Math.random() * 3),
    actual: Math.floor(Math.random() * 2),
    prevented: 1 + Math.floor(Math.random() * 2),
  }));
};

export default function PredictiveMaintenance() {
  const [selectedSystem, setSelectedSystem] = useState('BESS-001');
  const maintenanceHistory = useMemo(() => generateMaintenanceHistory(), []);
  ...
  const stats = useMemo(() => {
    ...
    const totalPrevented = maintenanceHistory.reduce((sum, m) => sum + m.prevented, 0);
